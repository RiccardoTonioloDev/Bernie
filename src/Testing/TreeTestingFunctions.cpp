...
RBBSTreeFilterTest(const RBBSTree<SerializableObject> &tree, const std::string &expected, const std::string &testName) {
    ...
    for (typename std::vector<const SerializableObject *>::const_iterator cit = saved.begin();
         cit != saved.end(); ++cit) {
...
template void RBBSTreeFilterTest<Account>(const RBBSTree<SerializableObject> &tree, const std::string &expected,
                                          const std::string &testName);

template void RBBSTreeFilterTest<CryptoWallet>(const RBBSTree<SerializableObject> &tree, const std::string &expected,
                                               const std::string &testName);

template void RBBSTreeFilterTest<Contact>(const RBBSTree<SerializableObject> &tree, const std::string &expected,
                                          const std::string &testName);

template void RBBSTreeFilterTest<Note>(const RBBSTree<SerializableObject> &tree, const std::string &expected,
                                       const std::string &testName);

template void RBBSTreeFilterTest<CreditCard>(const RBBSTree<SerializableObject> &tree, const std::string &expected,
                                             const std::string &testName);

template void
RBBSTreeFilterTest<SerializableObject>(const RBBSTree<SerializableObject> &tree, const std::string &expected,
                                       const std::string &testName);

void RBBSTreeSearchTest(const std::vector<const SerializableObject *> input, const std::string &expected,
                        const std::string &testName) {
    std::string vectorFound("");
    for (typename std::vector<const SerializableObject *>::const_iterator cit = input.begin();
         cit != input.end(); ++cit) {
